...
func TestSearch(t *testing.T) {
	...
	for _, caze := range [][]interface{}{
		...
		ipStr, expected, _ := caze[0].(string), caze[1].(string), caze[2].(map[string]string)
		ipRange := table.SearchV4(ipStr)
		if ipRange == nil {
			t.Errorf("Expected to get [%s], but not found by [%s]", expected, ipStr)
			return
		}
		if table.StringOfV4(ipRange) != expected {
			t.Errorf("Expected to get [%s], but got [%s]", expected, table.StringOfV4(ipRange))
			return
		}
	...
	for _, caze := range [][]interface{}{
		...
		ipStr, expected, _ := caze[0].(string), caze[1].(string), caze[2].(map[string]string)
		ipRange := table.SearchV6(ipStr)
		if ipRange == nil {
			t.Errorf("Expected to get [%s], but not found by [%s]", expected, ipStr)
			return
		}
		if table.StringOfV6(ipRange) != expected {
			t.Errorf("Expected to get [%s], but got [%s]", expected, table.StringOfV6(ipRange))
			return
		}
		//if !reflect.DeepEqual(table.AreaOf(ipRange), expectedMap) {
		//	t.Errorf("Expected to get [%v], but got [%v]", expectedMap, table.AreaOf(ipRange))
...
func TestSearchMissing(t *testing.T) {
	...
	for _, missing := range []string{"223.242.64.289"} {
		if ipRange := table.SearchV4(missing); ipRange != nil {
			t.Errorf("Expected not to find, but got [%s]", table.StringOfV4(ipRange))
			return
		}
	}
}
...
func BenchmarkSearchV6(b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			table.SearchV6("2001:218:0:2000::147")
			//for _, ipStr := range ips {
			//	table.Search(ipStr)
			//}
		}
